...
// ── Multer en mémoire : le fichier n'est jamais écrit sur disque ──
// On l'envoie directement vers Cloudinary depuis le buffer mémoire.
// (multer-storage-cloudinary est abandonné/incompatible avec
// cloudinary v2, donc on fait l'upload nous-mêmes.)
const upload = multer({
...
// ── Upload d'un buffer vers Cloudinary (Promise wrapper) ──
function uploadVersCloudinary(buffer, nomOriginal) {
  return new Promise((resolve, reject) => {
    const publicId = `${Date.now()}_${nomOriginal
      .normalize('NFD')
      .replace(/[\u0300-\u036f]/g, '')
...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      // Envoi du buffer mémoire vers Cloudinary
      const resultat = await uploadVersCloudinary(
        req.file.buffer,
      ...
      const nouveau = {
        ...
        nomServeur: resultat.public_id, // utilisé pour la suppression Cloudinary
        taille:     req.file.size,
        type:       req.file.mimetype,
        url:        resultat.secure_url, // URL Cloudinary complète, déjà en HTTPS
...
router.delete('/:id/fichiers/:nomServeur', async (req, res) => {
  try {
    ...
    // Supprimer sur Cloudinary (pas sur disque, plus rien en local)
    try {
      await cloudinary.uploader.destroy(req.params.nomServeur, {
        resource_type: 'auto',
      });
    } catch (cloudErr) {
